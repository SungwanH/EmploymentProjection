function [eqm_nlpf_CROSS,approx_nlpf_CROSS] = NLPF_DD(params, starting_point, base_point, hat_fundamentals,initial_guess)
% Non-linear double-difference perefect foresight equilibrium
% 50 regions + 37 other countries
% Timing assumption: t=0 corresponds to the first time index in the code
...
while (ITER_DYN <= MAXIT) && (Ymax > TOL_NL)
    ...
    for tt=TIME-1:-1:3
        ...
        mu_dot = mu_base(:,:,tt)./mu_base(:,:,tt-1);
        mu_dot(isnan(mu_dot)) = 0;
        temp=sum(mu(:,:,tt-1).*mu_dot.*temp0,2); 
