...
%    Ldyn(:,:,1) = reshape(L0,(J),R);
    ...
    %%%%Solving for the new path of values in double differences%%%        
...
%    realwages_us_nuu=realwages_us_aux2.^(1/NU);
    rw_lev(:,:,1) = rw_lev_base(:,:,1); % This holds only when the initial period starts from steady state
    ...
    rw_lev_base_RJ = reshape(rw_lev_base,R*J,TIME);
    %realwages_us_nuu=exp((1/(1-CRRA_PAR)) * rw_lev_RJ.^(1-CRRA_PAR)).^(1/NU); % This term is due to CRRA utility
    realwages_us_nuu = ones(R*J,TIME);
    for t=1:TIME-1
        temp1 = ((1/(1-CRRA_PAR)) * rw_lev_RJ(:,t+1).^(1-CRRA_PAR));
        temp2 = ((1/(1-CRRA_PAR)) * rw_lev_RJ(:,t).^(1-CRRA_PAR));
        temp3 = ((1/(1-CRRA_PAR)) * rw_lev_base_RJ(:,t+1).^(1-CRRA_PAR));
        temp4 = ((1/(1-CRRA_PAR)) * rw_lev_base_RJ(:,t).^(1-CRRA_PAR));
        realwages_us_nuu(:,t+1)= (exp(temp1 - temp2) ./ exp(temp3 - temp4)).^(1/NU); % This term is due to CRRA utility
    end
