function [eqm_dgp_so, approx_dgp_so] = DGP_SO(params, eqm_nlpf, approx_nlpf)
% This code gives DGP+PF path using Second Order Approximation
...
for t1=1:ENDT+1
    ...
    [eqm_temp] = PBP_DYN_SO(params, t1, t1, E_T_hat_test, kappa_hat, L, V, eqm_nlpf, approx_nlpf);
%    eqm_temp.L(:,t1+1)
...
%save the outputs
eqm_dgp_so   = v2struct(Ldyn, L_belief_dgp, L_hat, v_hat, w_hat, p_hat, P_hat, pi_hat, mu_hat, X_hat, E_T_hat, ...
    L_dgp, w_dgp, p_dgp, P_dgp, pi_dgp, mu_dgp, X_dgp, X, VALjn00, wf00, pf00);
approx_dgp_so = v2struct(mu, pi, varrho, chi, zeta, lambda);

save('DATA/DGP_SO.mat', 'eqm_dgp_so', 'approx_dgp_so'); 
