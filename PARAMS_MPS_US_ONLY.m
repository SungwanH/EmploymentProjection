function [params] = PARAMS_MPS_US_ONLY(W_TRUE, DRAW_SD, NUM_SIM)
TIME    =50; 
TIME_SS = 50;
C=0;% # non-US of countries
R=50;% # of US states
...
% Elasticities
load('DATA/Base_year_three_sectors_US_only.mat', 'X','B')
ALPHAS = X; %consumption share
T = 0.22*ones(J,1);
THETA  = 1./T; 
...
UPDT_V_NL   = 0.05; %update speed for nonlinear value loop (lower value->conservative)
UPDT_W_NL   = 0.1; %update speed for nonlinear wage loop (lower value->conservative)
TOL_NL      = 1E-5;  %tolerance rate for nonlinear dynamic equilibrium (outerloop)
TOL_NL_TEMP = 1E-5;  %tolerance rate for nonlinear temporary equilibrium (inner loop)
...
%Check how simulations sum up to zero
%sum(E_T_hat_sim(1,5,1:TIME,1,1:NUM_SIM,1),5)
figure
hold on
title('Productivity Paths')
plot(1:TIME,permute(T_SIM(1,5,1:TIME,:,1),[4,3,2,1]),'--')
...
figure
hold on
title('Log deviation in belief by time for simulation 1 (sd=0.1)')
...
%saveas(gcf,'FIGURES/E_T_hat_by_time_01.png')
%saveas(gcf,'FIGURES/E_T_hat_by_time_01.fig')
