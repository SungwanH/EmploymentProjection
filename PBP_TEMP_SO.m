function [w_hat, p_hat, P_hat, pi_hat, X_hat] = PBP_TEMP_SO(params, t1, T_hat, kappa_hat, L_hat_R, approx)
        ...
        for j=1:J
            for n=1:N
                for k=1:N
                    ...
                    %{
                    %P_pp1(j,n,k)= -THETA(j) * (1 -GAMMA(j,k))^2 * pi(n+(j-1)*N,k) * p_old(j,k) * p_old(j,k);
                    %P_pp2(j,n,k)= THETA(j) * (1-GAMMA(j,k)) * pi(k+(j-1)*N,k) * p_old(j,k) * p_old(j,k);
                    P_ww1(j,n,k) = (GAMMA(j,k) * (-THETA(j)) * GAMMA(j,k) * pi(n+(j-1)*N,k,t)) * w_hat(j,k,t) * w_hat(j,k,t);
                    P_pp1(j,n,k) = ((-THETA(j)) * (1-GAMMA(j,k)) * (1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t)) * p_old(j,k,t) * p_old(j,k,t);
                    P_TT1(j,n,k) = ((-1/THETA(j)) * pi(n+(j-1)*N,k,t) ) * T_hat(j,k,t) * T_hat(j,k,t);
                    P_wT1(j,n,k) = ( GAMMA(j,k) * pi(n+(j-1)*N,k,t))* w_hat(j,k,t) * T_hat(j,k,t);
                    P_pT1(j,n,k) = ((1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t)) * p_old(j,k,t) * T_hat(j,k,t);
                    P_wp1(j,n,k) = (GAMMA(j,k) * (1-GAMMA(j,k))* (-THETA(j)) * pi(n+(j-1)*N,k,t)) * w_hat(j,k,t) * p_old(j,k,t);
                    %}
                    for m=1:N
                        %diff w.r.t w and w
                        P_ww(j,n,k,m) = ((m==k) *  GAMMA(j,k) * GAMMA(j,k) *(-THETA(j)) * pi(n+(j-1)*N,k,t) -  GAMMA(j,k) * GAMMA(j,m) * (-THETA(j)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * w_hat(j,k,t) * w_hat(j,m,t);
                       %diff w.r.t p and p
                        P_pp(j,n,k,m) = ((m==k) * (-THETA(j)) * (1-GAMMA(j,k)) * (1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t) - (-THETA(j)) * (1-GAMMA(j,k)) * (1-GAMMA(j,m)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * p_old(j,k,t) * p_old(j,m,t);
                        %diff w.r.t T and T
                        P_TT(j,n,k,m) = ((m==k) *(-1/THETA(j)) * pi(n+(j-1)*N,k,t) - (-1/THETA(j)) * pi(n+(j-1)*N,k,t)* pi(n+(j-1)*N,m,t)) * T_hat(j,k,t) * T_hat(j,m,t);
                        %diff w.r.t w and T
                        P_wT(j,n,k,m) = ((m==k) * GAMMA(j,k) * pi(n+(j-1)*N,k,t) - GAMMA(j,k) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * w_hat(j,k,t) * T_hat(j,m,t);
                        %diff w.r.t p and T
                        P_pT(j,n,k,m) = ((m==k) * (1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t) - (1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t) ) * p_old(j,k,t) * T_hat(j,m,t);
                        %diff w.r.t w and P
                        P_wp(j,n,k,m) = ((m==k) * GAMMA(j,k) * (1-GAMMA(j,m))* (-THETA(j)) * pi(n+(j-1)*N,m,t) - (GAMMA(j,k)) * (1-GAMMA(j,m)) * (-THETA(j)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * w_hat(j,k,t) * p_old(j,m,t);
                        %P_ww2(j,n,k,m) = (-(-THETA(j)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t) * GAMMA(j,k) * GAMMA(j,m)) * w_hat(j,k,t) * w_hat(j,m,t);
                        %P_pp2(j,n,k,m) = (-(-THETA(j)) * (1-GAMMA(j,k)) * (1-GAMMA(j,m)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * p_old(j,k,t) * p_old(j,m,t);
                        %P_TT2(j,n,k,m) = (- (-1/THETA(j)) * pi(n+(j-1)*N,k,t)* pi(n+(j-1)*N,m,t)) * T_hat(j,k,t) * T_hat(j,m,t);
                        %P_wT2(j,n,k,m) = ( - GAMMA(j,k) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t))* w_hat(j,k,t) * T_hat(j,m,t);
                        %P_pT2(j,n,k,m) = (- (1-GAMMA(j,k)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t) ) * p_old(j,k,t) * T_hat(j,m,t);
                        %P_wp2(j,n,k,m) = ( - (GAMMA(j,k)) * (1-GAMMA(j,m)) * (-THETA(j)) * pi(n+(j-1)*N,k,t) * pi(n+(j-1)*N,m,t)) * w_hat(j,k,t) * p_old(j,m,t);
                    end
                end
            end
        end
        ...
        for j=1:J
            for i=1:N
                for n=1:N
                    ...
                    for m=1:N
                        %diff w.r.t. X and X
                        X_XX(j,i,n,m)   = ((n==m) * (1-GAMMA(j,i)) * varrho(m+(j-1)*N,i,t) - ((1-GAMMA(j,i))^2) * varrho(n+(j-1)*N,i,t)* varrho(m+(j-1)*N,i,t)) * X_hat(j,n,t) * X_hat(j,m,t);
                        ...
                        %diff w.r.t. pi and X
                        X_piX(j,i,n,m)  = ((n==m) * (1-GAMMA(j,i)) * varrho(m+(j-1)*N,i,t) - ((1-GAMMA(j,i))^2) * varrho(n+(j-1)*N,i,t)* varrho(m+(j-1)*N,i,t)) * pi_hat(n+(j-1)*N,i,t) * X_hat(j,m,t);
                    end
                    for s=1:J
                        X_piw(j,i,n,s) = -(1-GAMMA(j,i)) * ALPHAS(j,i) * varrho(n+(j-1)*N,i,t) * zeta(i+(s-1)*N,j,t) * pi_hat(n+(j-1)*N,i,t) * w_hat(s,i,t);
                        X_piL(j,i,n,s) = -(1-GAMMA(j,i)) * ALPHAS(j,i) * varrho(n+(j-1)*N,i,t) * zeta(i+(s-1)*N,j,t) * pi_hat(n+(j-1)*N,i,t) * L_hat(s,i,t);
                        X_Xw(j,i,n,s)  = -(1-GAMMA(j,i)) * ALPHAS(j,i) * varrho(n+(j-1)*N,i,t) * zeta(i+(s-1)*N,j,t) * X_hat(j,n,t) * w_hat(s,i,t);
                        X_XL(j,i,n,s)  = -(1-GAMMA(j,i)) * ALPHAS(j,i) * varrho(n+(j-1)*N,i,t) * zeta(i+(s-1)*N,j,t) * X_hat(j,n,t) * L_hat(s,i,t);
                    end
                end
                for k=1:J
                    %diff w.r.t. w
                    ...
                    for s=1:J
                        %diff w.r.t. w and w
                        X_ww(j,i,k,s) = ((k==s) * ALPHAS(j,i) * zeta(i+(k-1)*N,j,t) - (ALPHAS(j,i)^2) * zeta(i+(k-1)*N,j,t) * zeta(i+(s-1)*N,j,t)) * w_hat(k,i,t) * w_hat(s,i,t);
                        %diff w.r.t. L and L
                        X_LL(j,i,k,s) = ((k==s) * ALPHAS(j,i) * zeta(i+(k-1)*N,j,t) - (ALPHAS(j,i)^2) * zeta(i+(k-1)*N,j,t) * zeta(i+(s-1)*N,j,t)) * L_hat(k,i,t) * L_hat(s,i,t);
                        %diff w.r.t. w and L
                        X_wL(j,i,k,s) = ((k==s) * ALPHAS(j,i) * zeta(i+(k-1)*N,j,t) - (ALPHAS(j,i)^2) * zeta(i+(k-1)*N,j,t) * zeta(i+(s-1)*N,j,t)) * w_hat(k,i,t) * L_hat(s,i,t);
                    end
                end
            end
        end
        X_hat_temp(:,:,t) = sum(X_pi+X_X,3) +sum(X_w+X_L,3) + (1/2) * (sum(sum(X_XX+X_pipi,4),3) + sum(sum(X_ww+X_LL,4),3)) + sum(sum(X_piX,4),3) + sum(sum(X_piw+X_piL+X_Xw+X_XL,4),3) +  sum(sum(X_wL,4),3);
            ...
            for i=1:N
                for j=1:J
                    ...
                    for m=1:N
                        ...
                        w_XX(j,i,n,m)   =  ((m==n) * GAMMA(j,i) * chi(n+(j-1)*N,i,t) - (GAMMA(j,i)^2) * chi(n+(j-1)*N,i,t) * chi(m+(j-1)*N,i,t)) * X_hat(j,n,t) * X_hat(j,m,t);
                        w_piX(j,i,n,m)  =  ((m==n) * GAMMA(j,i) * chi(n+(j-1)*N,i,t) - (GAMMA(j,i)^2) * chi(n+(j-1)*N,i,t) * chi(m+(j-1)*N,i,t)) * pi_hat(n+(j-1)*N,i,t) * X_hat(j,m,t);
                    end
                end
            end
        end
